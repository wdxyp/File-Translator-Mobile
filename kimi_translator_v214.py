...
def configure_kimi(api_key=None, model=None, api_url=None):
    global KIMI_API_KEY, KIMI_MODEL, API_URL
    if api_key:
        KIMI_API_KEY = str(api_key).strip()
    if model:
        KIMI_MODEL = str(model).strip()
    if api_url:
...
def translate_file(input_file, output_dir=None, name="", direction="ko2zh", append=False, corpus=False, revision_file="revision.md", api_key=None, model=None, api_url=None):
    ...
    configure_kimi(api_key=api_key, model=model, api_url=api_url)
